	...
	<jsp:attribute name="content">
				...
				<fieldset>
					...
					<div style="display: flex">
						<div style="width: 330px;border-right:1px solid #e5e5e5; margin-right: 15px;">
							...
							<c:forEach items="${raid.getPlayers('membre', 'test', 'casu')}" var="p" varStatus="s">
								<div class="pure-g">
		...
		<%-- 								${p.player.rank.fr} --%>
						...
						<div style="width: 330px;border-right:1px solid #e5e5e5;margin-right: 15px;">
							...
							<c:forEach items="${raid.getPlayers('pu')}" var="p" varStatus="s">
								<div class="pure-g">
									<input type="hidden" name="players[${s.index}].player.id" value="${p.player.id}">
									<label class="pure-u-12-24">
										<input type="checkbox" name="players[${s.index}].member" <c:if test="${p.member}"> checked="checked"</c:if>>
		...
		<%-- 								${p.player.rank.fr} --%>
						...
						<div style="width: 400px">
							...
							<c:forEach items="${raid.getPlayers('reroll')}" var="p" varStatus="s">
								<div class="pure-g">
									<input type="hidden" name="players[${s.index}].player.id" value="${p.player.id}">
